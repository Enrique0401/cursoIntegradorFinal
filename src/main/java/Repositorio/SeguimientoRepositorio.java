...
public class SeguimientoRepositorio {
    ...
    /*public Seguimiento verPorEmail(String email) {
        String sql = "SELECT * FROM Cliente WHERE LOWER(email_cliente) = LOWER(?)";
        ...
        try (Connection conn = conexionDB.establecerConexion(); PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            try (ResultSet rs = ps.executeQuery()) {
                if (rs.next()) {
                    ...
                    seguimiento.setIdCliente(rs.getInt("id_cliente"));
                    seguimiento.setNombreCliente(rs.getString("nombre_cliente"));
                    seguimiento.setRucCliente(rs.getString("ruc_cliente"));
                    seguimiento.setDireccionCliente(rs.getString("direccion_cliente"));
                    seguimiento.setTelefonoCliente(rs.getString("telefono_cliente"));
                    seguimiento.setEmailCliente(rs.getString("email_cliente"));
                    seguimiento.setContrasenaCliente(rs.getString("contrasena_cliente"));
                    seguimiento.setRol(rs.getString("rol"));

                    // Si la base de datos guarda fecha_registro_cliente
                    try {
                        seguimiento.setFechaRegistro(rs.getTimestamp("fecha_registro_cliente").toLocalDateTime());
                    } catch (Exception e) {
                        seguimiento.setFechaRegistro(null);
                    }
